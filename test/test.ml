...
let test_simple () =
  ...
  let%bind result =
    ...
    |> Deferred.Or_error.List.iter ~how:`Parallel ~f:(fun () ->
           Thread_pool.with' pool worker)
  in
  Alcotest.(check unit_or_error)
...
let test_retry () =
  ...
  let%bind pool = Thread_pool.init ~name:"unittest" ~threads ~create ~destroy in
  let%bind _result =
    List.init work_expected ~f:ignore
    |> Deferred.Or_error.List.iter ~how:`Parallel ~f:(fun () ->
           Thread_pool.with' ~retries:1 pool (worker (ref true)))
  in
  let%bind () = Thread_pool.destroy pool in
...
let test_retry_fail () =
  ...
  let%bind result =
    List.init work ~f:ignore
    |> Deferred.Or_error.List.iter ~how:`Parallel ~f:(fun () ->
           Thread_pool.with' ~retries:0 pool worker)
  in
  let%bind () = Thread_pool.destroy pool in
  Alcotest.(check int) "Tries but no retries" work !tries;
  Alcotest.(check unit_or_error)
    "Result should be an error"
    (Or_error.errorf "exact error does not matter")
    result;
  return ()
